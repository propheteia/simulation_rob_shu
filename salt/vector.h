...
public:
    ...
    f_inline void Rotate(float degree)
   	{
   		const double length = Length();
   		float deg = GetAngleDeg();
   		deg += degree;
   		x() = length * gCos(gDegToRad(deg));
   		y() = length * gSin(gDegToRad(deg));
   	}

   	f_inline Vector2f Rotated(float degree)
   	{
   		const double length = Length();
   		float deg = GetAngleDeg();
   		deg += degree;
   		return Vector2f(length * gCos(gDegToRad(deg)), length * gSin(gDegToRad(deg)));
   	}
};
